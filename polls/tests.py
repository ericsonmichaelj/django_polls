...
class QuestionMethodTests(TestCase):
	def test_was_published_recently_with_future_question(self):
		"""
		was published_recently() should return False for questions
...
def create_question(question_text,days):
	...
	the givn number of `days` offset to now (negative for questions published)
...
class QuestionViewTests(TestCase):
	...
	def test_index_view_with_a_past_question(self):
		...
		response = self.client.get(reverse('polls:index'))
		self.assertQuerysetEqual(response.context['latest_question_list'],['<Question: Past questions.>'])
...
class QuestionIndexDetailTests(TestCase):
	def test_detail_view_with_a_future_question(self):
		"""
		The detail view of a question with a pub date in the future should return a 404 not found.
		"""
	...
	def test_detail_view_with_a_past_question(self):
		"""
		The detail view of a question with a pub_date in the past should display the question's question_text
		"""
